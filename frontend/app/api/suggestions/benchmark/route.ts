...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/suggestions/benchmark`, {
      method: 'POST',
...
export async function GET(request: Request) {
  try {
    ...
    const tenantId = resolveTenantIdFromRequest(request)
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/suggestions/benchmark/${recommendation_id}`, {
      headers: mergeTenantHeaders(tenantId),
    })
    const data = await response.json()
    return NextResponse.json(data)
